...
parser.add_argument('--config', type=str, default='algorithms/config/config_run_experiments.yaml', help='Path to configuration YAML file')
...
PROMPT_DATASET_PRESETS = {'parti': {'path': 'nateraw/parti-prompts', 'split': 'train', 'prompt_columns': ('Prompt', 'prompt', 'Prompts', 'prompts'), 'category_columns': ('Category', 'category')}, 'parti_prompts': {'path': 'nateraw/parti-prompts', 'split': 'train', 'prompt_columns': ('Prompt', 'prompt', 'Prompts', 'prompts'), 'category_columns': ('Category', 'category')}, 'drawbench': {'path': 'sayakpaul/drawbench', 'split': 'train', 'prompt_columns': ('Prompts', 'Prompt', 'prompt', 'prompts'), 'category_columns': ('Category', 'category')}}
...
def _load_prompt_dataset(config):
    ...
    if preset is None:
        ...
        raise ValueError(f"Invalid prompt_dataset '{dataset_name}'. Expected one of: {valid_names}.")
    ...
    if not all_prompts_with_category:
        ...
        raise ValueError(f"No prompts found in dataset '{dataset_path}' split '{dataset_split}'. Tried prompt column(s): {', '.join(prompt_columns)}. Available columns: {available_columns}.")
    print(f'Loaded {len(all_prompts_with_category)} prompts from {dataset_path} ({dataset_split}) across {len(category_prompts)} categories.')
    if skipped:
...
def _parse_prompt_index_range(prompt_index_range, prompt_count):
    ...
    if end > prompt_count:
        raise ValueError(f'prompt_index_range end ({end}) exceeds the available prompt count ({prompt_count}).')
    return (start, end)

def build_selected_prompts(seed):
    if USE_ENTIRE_DATASET:
        prompt_index_range = _parse_prompt_index_range(PROMPT_INDEX_RANGE, len(all_prompts_with_category))
        if prompt_index_range is None:
        ...
        print(f'Selected {len(selected_prompts_with_category)} of {len(all_prompts_with_category)} prompts from {len(category_prompts)} categories for seed {seed}.')
        if prompt_index_range is not None:
            print(f'Using prompt_index_range [{start}, {end}) with original prompt numbers {start + 1}-{end}.')
        return [(prompt, category, prompt_index + 1) for prompt_index, (prompt, category) in enumerate(selected_prompts_with_category, start=start)]
    if SINGLE_PROMPT_PER_SEED:
        rng = random.Random(seed)
        selected_prompt = rng.choice(all_prompts_with_category)
        print(f'Selected 1 prompt for seed {seed} from {len(all_prompts_with_category)} total prompts across {len(category_prompts)} categories.')
    ...
    return [(prompt, category, prompt_index) for prompt_index, (prompt, category) in enumerate(selected_prompts_with_category, start=1)]
...
def _load_result_runs(experiment_dir):
    ...
    for entry in sorted(os.listdir(experiment_dir)):
        ...
        if os.path.exists(score_csv):
            ...
            runs.append({'path': csv_path, 'prompt_dir': prompt_dir, 'prompt': prompt or os.path.basename(prompt_dir), 'category': category or '', 'x_label': 'iteration', 'x': pd.to_numeric(df['iteration'], errors='coerce').to_numpy(dtype=float), 'time': pd.to_numeric(df['elapsed_time'], errors='coerce').to_numpy(dtype=float), 'aesthetic': _numeric_values(df, 'aesthetic_score'), 'clip': _numeric_values(df, 'clip_score'), 'image_reward': _numeric_values(df, 'image_reward_score'), 'hpsv2': _numeric_values(df, 'hpsv2_score'), 'pickscore': _numeric_values(df, 'pickscore_score'), 'jpeg_size': _numeric_values(df, 'jpeg_size_kb'), 'objective': _numeric_values(df, 'combined_loss'), 'vram': _numeric_values(df, 'peak_vram_mb'), 'population_metrics': None, 'objective_name': 'loss'})
        if os.path.exists(fitness_csv):
            ...
            prompt = _first_non_empty(df['prompt']) if 'prompt' in df.columns else None
            category = _first_non_empty(df['category']) if 'category' in df.columns else None
            runs.append({'path': csv_path, 'prompt_dir': prompt_dir, 'prompt': prompt or os.path.basename(prompt_dir), 'category': category or '', 'x_label': 'generation', 'x': pd.to_numeric(df['generation'], errors='coerce').to_numpy(dtype=float), 'time': pd.to_numeric(df['elapsed_time'], errors='coerce').to_numpy(dtype=float), 'aesthetic': _numeric_values(df, 'max_aesthetic_score'), 'clip': _numeric_values(df, 'max_clip_score'), 'image_reward': _numeric_values(df, 'max_image_reward_score'), 'hpsv2': _numeric_values(df, 'max_hpsv2_score'), 'pickscore': _numeric_values(df, 'max_pickscore_score'), 'jpeg_size': _numeric_values(df, 'min_jpeg_size_kb'), 'objective': _numeric_values(df, 'max_fitness'), 'vram': _numeric_values(df, 'peak_vram_mb'), 'population_metrics': {'aesthetic': _numeric_values(df, 'avg_aesthetic_score'), 'clip': _numeric_values(df, 'avg_clip_score'), 'image_reward': _numeric_values(df, 'avg_image_reward_score'), 'hpsv2': _numeric_values(df, 'avg_hpsv2_score'), 'pickscore': _numeric_values(df, 'avg_pickscore_score'), 'jpeg_size': _numeric_values(df, 'avg_jpeg_size_kb'), 'objective': _numeric_values(df, 'avg_fitness')}, 'objective_name': 'fitness'})
    cleaned_runs = []
    for run in runs:
        ...
        if valid.sum() == 0:
            continue
        metric_values = {metric: run[metric][valid] for metric in ('aesthetic', 'clip', 'image_reward', 'hpsv2', 'pickscore', 'jpeg_size', 'objective', 'vram')}
        population_metrics = run['population_metrics']
        if population_metrics is not None:
            population_metrics = {metric: values[valid] for metric, values in population_metrics.items()}
        cleaned_runs.append({**run, 'x': run['x'][valid], 'time': run['time'][valid], **metric_values, 'population_metrics': population_metrics})
    return cleaned_runs
...
def _compute_similarity_metrics(runs):
    ...
    try:
        ...
        for run in runs:
            cos_sim, ssim_value = _compute_similarity_for_run(run['prompt_dir'], clip_model, clip_preprocess, device)
...
def _build_stats(axis, values, axis_name):
    return pd.DataFrame({axis_name: axis, 'count': np.sum(~np.isnan(values), axis=0), 'avg': np.nanmean(values, axis=0), 'std': np.nanstd(values, axis=0), 'min': np.nanmin(values, axis=0), 'max': np.nanmax(values, axis=0)})
...
def _grid_metric_lines(run, index, objective_name, max_width, draw, font):
    metric_specs = [(objective_name.title(), 'objective'), ('Aes', 'aesthetic'), ('CLIP', 'clip'), ('IR', 'image_reward'), ('HPS', 'hpsv2'), ('Pick', 'pickscore'), ('JPEG KB', 'jpeg_size')]
    parts = []
...
def _save_aggregate_image_grids(runs, objective_name, experiment_dir, rows_per_page=20):
    ...
    for page_index in range(page_count):
        ...
        for run, initial_path, best_path in page_runs:
            ...
            initial_lines = ['Initial'] + _grid_metric_lines(run, 0, objective_name, tile_w, scratch_draw, metric_font)
            best_lines = ['Best'] + _grid_metric_lines(run, -1, objective_name, tile_w, scratch_draw, metric_font)
...
def _summary_rows(df, group_name):
    ...
    for metric in ['aesthetic', 'clip', 'image_reward', 'hpsv2', 'pickscore', 'jpeg_size', 'objective', 'peak_vram_mb', 'cosine_similarity', 'ssim']:
...
def _stats_summary_row(metric, solution_type, values):
    ...
    return {'metric': metric, 'solution_type': solution_type, 'count': int(values.size), 'min': float(np.min(values)) if values.size else np.nan, 'mean': float(np.mean(values)) if values.size else np.nan, 'median': float(np.median(values)) if values.size else np.nan, 'max': float(np.max(values)) if values.size else np.nan, 'std': float(np.std(values)) if values.size else np.nan}
...
def _final_metrics_workbook_frames(runs, objective_name):
    metric_labels = {'aesthetic': 'aesthetic', 'clip': 'clip', 'image_reward': 'image_reward', 'hpsv2': 'hpsv2', 'pickscore': 'pickscore', 'jpeg_size': 'jpeg_size_kb', 'objective': objective_name}
    value_rows = []
    all_sample_rows = []
    for run in runs:
        population_metrics = run['population_metrics']
        for metric, label in metric_labels.items():
            ...
            value_rows.append({'prompt': run['prompt'], 'category': run['category'], 'result_file': run['path'], 'elapsed_time_seconds': float(run['time'][-1]), 'peak_vram_mb': float(np.nanmax(run['vram'])) if np.isfinite(run['vram']).any() else np.nan, 'metric': label, 'population': population_value, 'best_solution': float(run[metric][-1])})
            ...
            for x_value, elapsed_time, sample_value in zip(run['x'], run['time'], sample_values):
                all_sample_rows.append({'prompt': run['prompt'], 'category': run['category'], 'result_file': run['path'], 'x_label': run['x_label'], 'x': float(x_value), 'elapsed_time_seconds': float(elapsed_time), 'metric': label, 'value': float(sample_value), 'source': 'generation_average' if population_metrics is not None else 'sample'})
...
def _aggregate_one_experiment(experiment_dir):
    ...
    if len(objective_names) != 1 or len(x_labels) != 1:
        raise ValueError('Mixed result schemas found. Run aggregation on one method folder at a time.')
    ...
    metric_labels = [('aesthetic', 'aesthetic'), ('clip', 'clip'), ('image_reward', 'image_reward'), ('hpsv2', 'hpsv2'), ('pickscore', 'pickscore'), ('jpeg_size', 'jpeg_size_kb'), ('objective', objective_name), ('vram', 'peak_vram_mb')]
    for metric, label in metric_labels:
        ...
        _plot_evolution(step_stats, axis_name, label, f'{label.title()} evolution by {axis_name}', os.path.join(experiment_dir, f'{label}_evolution_by_{axis_name}.jpg'))
        ...
        _plot_evolution(time_stats, 'elapsed_time_seconds', label, f'{label.title()} evolution by elapsed time', os.path.join(experiment_dir, f'{label}_evolution_by_time.jpg'))
    ...
    for run, cos_sim, ssim_value in zip(runs, cosine_similarity, ssim_vals):
        final_rows.append({'prompt': run['prompt'], 'category': run['category'], 'aesthetic': float(run['aesthetic'][-1]), 'clip': float(run['clip'][-1]), 'image_reward': float(run['image_reward'][-1]), 'hpsv2': float(run['hpsv2'][-1]), 'pickscore': float(run['pickscore'][-1]), 'jpeg_size': float(run['jpeg_size'][-1]), 'objective': float(run['objective'][-1]), 'peak_vram_mb': float(np.nanmax(run['vram'])) if np.isfinite(run['vram']).any() else np.nan, 'cosine_similarity': cos_sim, 'ssim': ssim_value})
    ...
    _write_final_metrics_workbook(runs, objective_name, os.path.join(experiment_dir, 'aggregate_final_metrics.xlsx'))
    ...
    for seed in seed_list:
        ...
        for prompt, category, prompt_number in selected_prompts_with_category:
            ...
            if config['optimization_method'] == 'ga':
                ...
                if config['optimization_method'] == 'gomea':
