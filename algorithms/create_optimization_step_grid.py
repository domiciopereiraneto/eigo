...
def load_font(size: int, bold: bool=False) -> ImageFont.ImageFont:
    names = ('DejaVuSans-Bold.ttf', 'LiberationSans-Bold.ttf', 'Arial Bold.ttf') if bold else ('DejaVuSans.ttf', 'LiberationSans-Regular.ttf', 'Arial.ttf')
    for name in names:
        try:
...
def wrap_text(draw: ImageDraw.ImageDraw, text: str, font: ImageFont.ImageFont, max_width: int, max_lines: Optional[int]=None) -> list[str]:
...
def draw_centered_text(draw: ImageDraw.ImageDraw, lines: Sequence[str], font: ImageFont.ImageFont, x: int, y: int, width: int, fill: tuple[int, int, int], line_gap: int=2) -> int:
    cursor = y
    for line in lines:
...
def available_population_ids(prompt_dir: Path, generation: int) -> list[int]:
    ...
    for image_path in gen_dir.iterdir():
        if not image_path.is_file() or image_path.suffix.lower() not in IMAGE_EXTENSIONS:
            continue
        match = re.fullmatch('id_(\\d+)', image_path.stem)
        if match:
...
def build_cells(prompt_dir: Path, algo: str, steps: Sequence[int], population_ids: Sequence[int], include_baseline: bool, use_best_for_population_methods: bool) -> list[Cell]:
    cells: list[Cell] = []
    if include_baseline:
...
def create_grid(rows: Sequence[PromptRow], output_path: Path, tile_size: tuple[int, int], prompt_width: int, column_gap: int, row_gap: int, margin: int, header_height: int, prompt_font_size: int, header_font_size: int) -> None:
    if not rows:
    ...
    for col_idx, cell in enumerate(first_cells):
        ...
        draw_centered_text(draw, label_lines, header_font, x0 + col_idx * (tile_w + column_gap), margin + 8, tile_w, (0, 0, 0))
...
def parse_size(value: str) -> tuple[int, int]:
    match = re.fullmatch('(\\d+)x(\\d+)', value.lower())
    if not match:
...
def main(argv: Optional[Sequence[str]]=None) -> int:
    parser = argparse.ArgumentParser(description='Generate image grids with prompts as rows and optimization steps as columns. Steps map to gen_N/id_M images unless --best is used.')
    parser.add_argument('--config', default=str(DEFAULT_CONFIG), help=f'YAML config path. Defaults to {DEFAULT_CONFIG.relative_to(REPO_ROOT)}.')
    ...
    for run_dir in run_dirs:
        ...
        for prompt_index, prompt_dir in prompt_dirs:
            cells = build_cells(prompt_dir=prompt_dir, algo=algo, steps=steps, population_ids=population_ids, include_baseline=include_baseline, use_best_for_population_methods=use_best)
            if not cells:
        ...
        create_grid(rows=rows, output_path=output_path, tile_size=tile_size, prompt_width=prompt_width, column_gap=column_gap, row_gap=row_gap, margin=margin, header_height=header_height, prompt_font_size=prompt_font_size, header_font_size=header_font_size)
