...
METRIC_NAMES: Tuple[str, ...] = ('aesthetic_score', 'clip_score', 'image_reward_score', 'hpsv2_score', 'pickscore_score', 'jpeg_size_kb')
METRIC_ALIASES = {'all': 'all', 'aesthetic': 'aesthetic_score', 'aesthetic_score': 'aesthetic_score', 'clip': 'clip_score', 'clip_score': 'clip_score', 'image_reward': 'image_reward_score', 'imagereward': 'image_reward_score', 'image_reward_score': 'image_reward_score', 'hps': 'hpsv2_score', 'hpsv2': 'hpsv2_score', 'hpsv2_score': 'hpsv2_score', 'pickscore': 'pickscore_score', 'pick_score': 'pickscore_score', 'pickscore_score': 'pickscore_score', 'jpeg': 'jpeg_size_kb', 'jpg': 'jpeg_size_kb', 'jpeg_size': 'jpeg_size_kb', 'jpeg_size_kb': 'jpeg_size_kb'}
POPULATION_COLUMNS = {'fitness': ('avg_fitness', 'std_fitness', 'max_fitness'), 'aesthetic_score': ('avg_aesthetic_score', 'std_aesthetic_score', 'max_aesthetic_score'), 'clip_score': ('avg_clip_score', 'std_clip_score', 'max_clip_score'), 'image_reward_score': ('avg_image_reward_score', 'std_image_reward_score', 'max_image_reward_score'), 'hpsv2_score': ('avg_hpsv2_score', 'std_hpsv2_score', 'max_hpsv2_score'), 'pickscore_score': ('avg_pickscore_score', 'std_pickscore_score', 'max_pickscore_score'), 'jpeg_size_kb': ('avg_jpeg_size_kb', 'std_jpeg_size_kb', 'min_jpeg_size_kb')}
...
def sample_image_path(prompt_dir: Path, sample_id: int, sample_seed) -> Optional[Path]:
    ...
    if seed_text is not None:
        stems.extend([f'sample_{sample_id}_seed_{seed_text}', f'sample_{sample_id}_latent_seed_{seed_text}'])
    for stem in stems:
        path = result_image_path(prompt_dir, stem)
        if path is not None:
            return path
    matches = sorted((path for path in prompt_dir.glob(f'sample_{sample_id}_*.jpg') if path.is_file()))
...
def prompt_dirs(experiment_dir: Path) -> List[Path]:
    return [path for path in sorted(experiment_dir.iterdir()) if path.is_dir() and path.name.startswith('results_')]
...
class MetricBackfiller:
    ...
    def __init__(self, base_config: dict, metrics_to_calculate: Sequence[str], cuda_override=None, scorer_results_folder: Optional[Path]=None):
    ...
    def config_for_prompt_dir(self, prompt_dir: Path, df: pd.DataFrame) -> dict:
        ...
        config['selected_prompt'] = config.get('selected_prompt') or first_non_empty(df, 'prompt') or prompt_dir.name
        config['results_folder'] = str(self.scorer_results_folder)
        required_defaults = {'optimization_method': 'random_sampler', 'optimization_target': 'text_tokens', 'model_id': 'stable-diffusion-v1-5/stable-diffusion-v1-5', 'aesthetic_predictor': 2, 'seed': 42, 'cuda': 'cpu'}
        for key, value in required_defaults.items():
            config.setdefault(key, value)
        return config

    def engine_for_config(self, config: dict):
        key = tuple((str(config.get(name)) for name in ('model_id', 'base_model_id', 'unet_model_id', 'unet_subfolder', 'model_backend', 'torch_dtype', 'cuda', 'aesthetic_predictor', 'clip_model_name', 'image_reward_model', 'image_reward_model_name', 'hpsv2_version', 'pickscore_model', 'pickscore_model_name', 'pickscore_processor', 'pickscore_processor_name', 'aesthetic_score_weight', 'clip_score_weight', 'image_reward_score_weight', 'hpsv2_score_weight', 'pickscore_score_weight', 'jpeg_size_weight', 'max_aesthetic_score', 'max_clip_score', 'max_image_reward_score', 'max_hpsv2_score', 'max_pickscore_score', 'max_jpeg_size_kb', 'jpeg_quality', 'metrics_to_calculate')))
    ...
    def score_image(self, engine, image_path: Path, prompt: str) -> dict:
        ...
        combined_score, aesthetic_score, clip_score, image_reward_score, hpsv2_score, pickscore_score, jpeg_size_kb, _ = engine._evaluate_canonical_image_path_scores(str(image_path), prompt)
        scores = {'fitness': float(combined_score), 'aesthetic_score': float(aesthetic_score), 'clip_score': float(clip_score), 'image_reward_score': float(image_reward_score), 'hpsv2_score': float(hpsv2_score), 'pickscore_score': float(pickscore_score), 'jpeg_size_kb': float(jpeg_size_kb)}
    ...
    def _population_image_paths(self, prompt_dir: Path, generation: int) -> List[Path]:
        ...
        if gen_dir.is_dir():
            paths = sorted((path for path in gen_dir.iterdir() if path.suffix.lower() in {'.jpg', '.jpeg', '.png'}))
            if paths:
                return paths
        if generation == 0:
    ...
    def _random_sampler_image_paths(self, prompt_dir: Path, row: pd.Series, sample_df: pd.DataFrame, cumulative: bool=False) -> List[Path]:
        ...
        if cumulative:
            sample_start = 0
        selected_rows = sample_df[(pd.to_numeric(sample_df['sample'], errors='coerce') >= sample_start) & (pd.to_numeric(sample_df['sample'], errors='coerce') <= sample_end)]
        image_paths = []
        for _, sample_row in selected_rows.iterrows():
            ...
            if image_path is None:
                raise FileNotFoundError(f"Missing random sampler image for sample {int(sample_row['sample'])} under {prompt_dir}")
    ...
    def backfill_fitness_csv(self, prompt_dir: Path, csv_path: Path) -> pd.DataFrame:
        ...
        for row_idx, row in df.iterrows():
            ...
            image_paths = self._random_sampler_image_paths(prompt_dir, row, sample_df) if is_random_sampler else self._population_image_paths(prompt_dir, generation)
            if not image_paths:
            ...
            for metric in metrics_for_row:
                ...
                if is_random_sampler:
                    ...
                    best_values = np.asarray([self.score_image(engine, image_path, prompt)[metric] for image_path in cumulative_paths], dtype=float)
...
def aggregate_experiment(experiment_dir: Path, csv_paths: Sequence[Path], csv_name: str, excel_name: str) -> None:
    value_rows = []
    sample_rows = []
    metric_specs = [('aesthetic_score', 'aesthetic', 'aesthetic_score', 'max_aesthetic_score', 'avg_aesthetic_score'), ('clip_score', 'clip', 'clip_score', 'max_clip_score', 'avg_clip_score'), ('image_reward_score', 'image_reward', 'image_reward_score', 'max_image_reward_score', 'avg_image_reward_score'), ('hpsv2_score', 'hpsv2', 'hpsv2_score', 'max_hpsv2_score', 'avg_hpsv2_score'), ('pickscore_score', 'pickscore', 'pickscore_score', 'max_pickscore_score', 'avg_pickscore_score'), ('jpeg_size_kb', 'jpeg_size_kb', 'jpeg_size_kb', 'min_jpeg_size_kb', 'avg_jpeg_size_kb')]
    for csv_path in csv_paths:
        ...
        for raw_name, label, score_col, fitness_col, avg_col in metric_specs:
            ...
            value_rows.append({'prompt': prompt, 'category': category, 'result_file': str(csv_path), 'elapsed_time_seconds': elapsed, 'peak_vram_mb': float(peak_vram) if np.isfinite(peak_vram) else np.nan, 'metric': label, 'population': np.nan if is_score else float(final.get(avg_col, np.nan)), 'best_solution': float(final.get(best_col, np.nan))})
            for _, row in df.iterrows():
                sample_rows.append({'prompt': prompt, 'category': category, 'result_file': str(csv_path), 'x_label': step_col, 'x': float(row[step_col]), 'elapsed_time_seconds': float(row.get('elapsed_time', np.nan)), 'metric': label, 'value': float(row.get(sample_col, np.nan)), 'source': 'sample' if is_score else 'generation_average'})
        value_rows.append({'prompt': prompt, 'category': category, 'result_file': str(csv_path), 'elapsed_time_seconds': elapsed, 'peak_vram_mb': float(peak_vram) if np.isfinite(peak_vram) else np.nan, 'metric': objective_label, 'population': np.nan if is_score else float(final.get(objective_sample_col, np.nan)), 'best_solution': float(final.get(objective_best_col, np.nan))})
        for _, row in df.iterrows():
            sample_rows.append({'prompt': prompt, 'category': category, 'result_file': str(csv_path), 'x_label': step_col, 'x': float(row[step_col]), 'elapsed_time_seconds': float(row.get('elapsed_time', np.nan)), 'metric': objective_label, 'value': float(row.get(objective_sample_col, np.nan)), 'source': 'sample' if is_score else 'generation_average'})
...
def run_batch_aggregate_one_experiment(experiment_dir: Path, base_config_path: Path) -> None:
    ...
    try:
        sys.argv = [str(REPO_ROOT / 'algorithms' / 'run_experiments.py'), '--config', str(base_config_path)]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Backfill zero-weight image metrics in existing EIGO experiment folders.')
    parser.add_argument('--config', default='algorithms/config/config_zero_weight_metrics.yaml', help='Path to the YAML backfill config.')
...
def main() -> None:
    ...
    backfiller = MetricBackfiller(base_config, metrics_to_calculate=metrics_to_calculate, cuda_override=cuda_override, scorer_results_folder=Path(scorer_results_folder))
    for experiment_dir in experiment_dirs:
        ...
        for prompt_dir in prompt_dirs(experiment_dir):
            for csv_name, handler in (('score_results.csv', backfiller.backfill_score_csv), ('fitness_results.csv', backfiller.backfill_fitness_csv)):
        ...
        if completed_csvs:
            if overwrite:
                ...
                aggregate_experiment(experiment_dir, completed_csvs, str(config.get('aggregate_csv_name', 'aggregate_final_metrics_completed.csv')), str(config.get('aggregate_excel_name', 'aggregate_final_metrics_completed.xlsx')))
