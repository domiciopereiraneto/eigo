...
from process_quantitative_results import CANONICAL_METRIC_COLUMNS, discover_experiment_dirs, experiment_metadata, load_prompt_config, load_yaml, prompt_dirs, read_result_csv, clean_result_df, resolve_path
LOWER_IS_BETTER = {'jpeg_size_kb', 'combined_loss'}
DEFAULT_METRICS = ('objective', 'aesthetic_score', 'clip_score', 'image_reward_score', 'hpsv2_score', 'pickscore_score', 'jpeg_size_kb')
DEFAULT_METRIC_LABELS = {'objective': 'Objective', 'aesthetic_score': 'Aesthetic score', 'clip_score': 'CLIP score', 'image_reward_score': 'ImageReward score', 'hpsv2_score': 'HPSv2 score', 'pickscore_score': 'PickScore', 'jpeg_size_kb': 'JPEG size (KB)', 'elapsed_time': 'Elapsed time (s)', 'peak_vram_mb': 'Peak VRAM (MB)'}
MUTED_COLORS = ('#6f8fd8', '#86a96f', '#e6a657', '#9b8cc9', '#c77b7b', '#6aa7a8', '#b8a35e', '#8f8f8f')
...
def safe_name(text: str) -> str:
    return re.sub('[^a-zA-Z0-9._-]+', '_', str(text)).strip('_') or 'metric'
...
def population_size(run_config: dict, metadata: dict) -> int:
    ...
    keys_by_method = {'gomea': ('gomea_pop_size', 'pop_size'), 'ga': ('ga_pop_size', 'pop_size'), 'randomsampler': ('num_images_to_generate', 'pop_size'), 'random_sampler': ('num_images_to_generate', 'pop_size')}
...
def nfe_from_clean_curve(clean: pd.DataFrame, csv_kind: str, pop_size: int, diffusion_steps: int) -> np.ndarray:
    if 'evaluations' in clean.columns:
        return (clean['evaluations'].to_numpy(dtype=float) + 1) * float(diffusion_steps)
...
def clean_curve_arrays(df: pd.DataFrame, step_column: str, value_column: str, metric: str, best_so_far: bool, csv_kind: str, pop_size: int, diffusion_steps: int):
    ...
    clean_data = {'step': step[valid].to_numpy(dtype=float), 'elapsed_time': elapsed[valid].to_numpy(dtype=float), 'value': value[valid].to_numpy(dtype=float)}
    ...
    if 'evaluations' in df.columns:
        clean_data['evaluations'] = pd.to_numeric(df['evaluations'], errors='coerce')[valid].to_numpy(dtype=float)
    clean = pd.DataFrame(clean_data).sort_values('step')
    clean = clean.drop_duplicates(subset=['step'], keep='last')
    if clean.empty:
        return None
    return (clean['step'].to_numpy(dtype=float), nfe_from_clean_curve(clean, csv_kind, pop_size, diffusion_steps), clean['elapsed_time'].to_numpy(dtype=float), apply_best_so_far(metric, clean['value'].to_numpy(dtype=float), best_so_far))
...
def load_experiment_curves(experiment_dirs: Sequence[Path], metric: str, label_template: str, best_so_far: bool, label_overrides: Optional[Dict[Path, str]]=None) -> List[ExperimentCurves]:
    ...
    for experiment_dir in experiment_dirs:
        ...
        for prompt_dir in prompt_dirs(experiment_dir):
            ...
            arrays = clean_curve_arrays(df, step_column, column, metric, best_so_far, csv_kind, pop_size, diffusion_steps)
            if arrays is None:
                continue
            step, nfe, elapsed_time, value = arrays
            csv_kinds.add(csv_kind)
            curves.append(PromptCurve(experiment_label=label, results_folder_name=metadata['results_folder_name'], csv_kind=csv_kind, prompt_folder=prompt_dir.name, step=step, nfe=nfe, elapsed_time=elapsed_time, value=value))
        if curves:
            experiments.append(ExperimentCurves(path=experiment_dir, label=label, metadata=metadata, csv_kinds=csv_kinds, curves=curves))
    return experiments
...
def native_nfe_stats(experiment: ExperimentCurves) -> pd.DataFrame:
    ...
    for curve in experiment.curves:
        ...
        rows.extend(({'nfe': float(nfe), 'value': float(value)} for nfe, value in zip(curve.nfe[valid], curve.value[valid])))
    if not rows:
        return pd.DataFrame(columns=['nfe', 'mean', 'std', 'count'])
    values = pd.DataFrame(rows)
    return values.groupby('nfe', as_index=False).agg(mean=('value', 'mean'), std=('value', lambda series: series.std(ddof=0)), count=('value', 'count')).sort_values('nfe').reset_index(drop=True)
...
def native_time_stats(experiment: ExperimentCurves, time_limit: Optional[float]) -> pd.DataFrame:
    ...
    for curve in experiment.curves:
        ...
        rows.extend(({'nfe': float(nfe), 'elapsed_time': float(elapsed), 'value': float(value)} for nfe, elapsed, value in zip(curve.nfe[valid], curve.elapsed_time[valid], curve.value[valid])))
    if not rows:
        return pd.DataFrame(columns=['elapsed_time_seconds', 'mean', 'std', 'count', 'nfe'])
    values = pd.DataFrame(rows)
    return values.groupby('nfe', as_index=False).agg(elapsed_time_seconds=('elapsed_time', 'mean'), mean=('value', 'mean'), std=('value', lambda series: series.std(ddof=0)), count=('value', 'count')).sort_values('elapsed_time_seconds').reset_index(drop=True)

def discover_dirs_with_optional_labels(folder_values: Sequence, label_values: Sequence, folder_key: str, label_key: str) -> Tuple[List[Path], Dict[Path, str]]:
    experiment_dirs, labels_by_dir, _ = discover_dirs_with_optional_labels_and_markers(folder_values, label_values, [], folder_key, label_key, '')
    return (experiment_dirs, labels_by_dir)

def discover_dirs_with_optional_labels_and_markers(folder_values: Sequence, label_values: Sequence, marker_values: Sequence, folder_key: str, label_key: str, marker_key: str) -> Tuple[List[Path], Dict[Path, str], Dict[Path, str]]:
...
def final_value_stats(experiment: ExperimentCurves) -> Optional[dict]:
    ...
    return {'mean': float(np.mean(values)), 'std': float(np.std(values, ddof=0)), 'count': int(values.size)}

def write_stats_csv(rows: List[dict], path: Path) -> None:
    pd.DataFrame(rows).to_csv(path, index=False, na_rep='nan')

def plot_stats(series: Dict[str, pd.DataFrame], x_column: str, x_label: str, y_label: str, title: str, out_path: Path, width: float, height: float, font_size: float, marker_size: float, xscale: str='linear', base10_ticks: bool=False, constant_series: Optional[Dict[str, dict]]=None, markers: Optional[Dict[str, str]]=None, y_multiplier: float=1.0, y_decimal_places: Optional[int]=None) -> None:
    ...
    for index, (label, stats_df) in enumerate(series.items()):
        ...
        ax.plot(xv, mv, marker=markers.get(label, 'o'), color=color, markersize=marker_size, markevery=marker_every, linewidth=2.8, label=label)
    for offset, (label, stats) in enumerate((constant_series or {}).items(), start=len(series)):
        ...
        ax.axhline(value, color=color, linestyle='--', linewidth=2.4, label=label)
    ...
    if y_decimal_places is not None:
        ax.yaxis.set_major_formatter(FuncFormatter(lambda value, _: f'{value:.{y_decimal_places}f}'))
...
def build_runcharts(config: dict) -> List[Path]:
    ...
    experiment_dirs, experiment_labels_by_dir, experiment_markers_by_dir = discover_dirs_with_optional_labels_and_markers(folder_values, experiment_label_values, experiment_marker_values, 'experiment_folders', 'experiment_labels', 'experiment_markers')
    ...
    if constant_folder_values:
        constant_experiment_dirs, constant_labels_by_dir = discover_dirs_with_optional_labels(constant_folder_values, constant_label_values, 'constant_experiment_folders', 'constant_labels')
    out_dir = resolve_path(config.get('save_folder', 'results_runcharts')) / 'runcharts'
    out_dir.mkdir(parents=True, exist_ok=True)
    if bool(config.get('clean_output', True)):
        for pattern in ('*_by_progress.csv', '*_by_progress.png', '*_by_nfe.csv', '*_by_nfe.png', '*_by_time.csv', '*_by_time.png'):
    ...
    for metric in metrics:
        metric = str(metric)
        experiments = load_experiment_curves(experiment_dirs, metric, label_template, best_so_far, experiment_labels_by_dir)
        if not experiments:
            print(f"Warning: no curves found for metric '{metric}'.")
            continue
        experiment_markers_by_label = {experiment.label: experiment_markers_by_dir[experiment.path] for experiment in experiments if experiment.path in experiment_markers_by_dir}
        constant_experiments = load_experiment_curves(constant_experiment_dirs, metric, label_template, best_so_far)
        constant_series = {}
        ...
        for experiment in constant_experiments:
            ...
            constant_series[label] = {**stats, 'results_folder_name': experiment.metadata['results_folder_name']}
        nfe_series = {}
        nfe_rows = []
        for experiment in experiments:
            ...
            for row in stats_df.itertuples(index=False):
                nfe_rows.append({'metric': metric, 'experiment': experiment.label, 'results_folder_name': experiment.metadata['results_folder_name'], 'nfe': row.nfe, 'mean': row.mean, 'std': row.std, 'count': row.count, 'constant_line': False})
        for label, stats in constant_series.items():
            nfe_rows.append({'metric': metric, 'experiment': label, 'results_folder_name': stats['results_folder_name'], 'nfe': np.nan, 'mean': stats['mean'], 'std': stats['std'], 'count': stats['count'], 'constant_line': True})
        ...
        plot_stats(nfe_series, 'nfe', 'Inference Compute (NFE)', plot_metric_label(metric, config), f'{plot_metric_label(metric, config)} by NFE', nfe_png, width, height, font_size, marker_size, nfe_xscale, True, constant_series, experiment_markers_by_label, y_multiplier, y_decimal_places)
        ...
        for experiment in experiments:
            ...
            for row in stats_df.itertuples(index=False):
                time_rows.append({'metric': metric, 'experiment': experiment.label, 'results_folder_name': experiment.metadata['results_folder_name'], 'time_limit_seconds': resolved_limit, 'elapsed_time_seconds': row.elapsed_time_seconds, 'nfe': row.nfe, 'mean': row.mean, 'std': row.std, 'count': row.count, 'constant_line': False})
        for label, stats in constant_series.items():
            time_rows.append({'metric': metric, 'experiment': label, 'results_folder_name': stats['results_folder_name'], 'time_limit_seconds': resolved_limit, 'elapsed_time_seconds': np.nan, 'nfe': np.nan, 'mean': stats['mean'], 'std': stats['std'], 'count': stats['count'], 'constant_line': True})
        time_csv = out_dir / f'{safe_metric}_by_time.csv'
        time_png = out_dir / f'{safe_metric}_by_time.png'
        write_stats_csv(time_rows, time_csv)
        plot_stats(time_series, 'elapsed_time_seconds', 'Elapsed time (s)', plot_metric_label(metric, config), f'{plot_metric_label(metric, config)} by elapsed time', time_png, width, height, font_size, marker_size, constant_series=constant_series, markers=experiment_markers_by_label, y_multiplier=y_multiplier, y_decimal_places=y_decimal_places)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--config', default='algorithms/config/config_process_runchart_results.yaml', help='YAML config containing experiment folders, metrics, and plotting options.')
