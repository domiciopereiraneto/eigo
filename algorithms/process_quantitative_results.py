...
DEFAULT_METHOD_LABELS = {'gomea': 'GOMEA', 'ga': 'GA', 'randomsampler': 'Random sampler', 'random_sampler': 'Random sampler'}
WEIGHT_TO_METRIC = {'aesw': 'aesthetic_score', 'clipw': 'clip_score', 'irw': 'image_reward_score', 'hpsw': 'hpsv2_score', 'psw': 'pickscore_score', 'jpgw': 'jpeg_size_kb'}
CANONICAL_METRIC_COLUMNS = {'score': {'objective': 'combined_score', 'aesthetic_score': 'aesthetic_score', 'clip_score': 'clip_score', 'image_reward_score': 'image_reward_score', 'hpsv2_score': 'hpsv2_score', 'pickscore_score': 'pickscore_score', 'jpeg_size_kb': 'jpeg_size_kb', 'elapsed_time': 'elapsed_time', 'peak_vram_mb': 'peak_vram_mb'}, 'fitness': {'objective': 'best_fitness', 'aesthetic_score': 'max_aesthetic_score', 'clip_score': 'max_clip_score', 'image_reward_score': 'max_image_reward_score', 'hpsv2_score': 'max_hpsv2_score', 'pickscore_score': 'max_pickscore_score', 'jpeg_size_kb': 'min_jpeg_size_kb', 'elapsed_time': 'elapsed_time', 'peak_vram_mb': 'peak_vram_mb'}}
METADATA_COLUMNS = {'generation', 'iteration', 'prompt', 'category', 'elapsed_time', 'peak_vram_mb'}

def load_yaml(path: Path) -> dict:
    if yaml is None:
        raise RuntimeError('PyYAML is required to read config files. Install project requirements or run `pip install PyYAML`.')
...
def parse_weights(name: str) -> Dict[str, int]:
    ...
    for short_name, metric in WEIGHT_TO_METRIC.items():
        match = re.search(f'(?:^|_){short_name}(-?\\d+)(?:_|$)', lowered)
        if match:
            weights[metric] = int(match.group(1))
    old_style = re.search('(?:^|_)a(\\d+)_b(\\d+)(?:_|$)', lowered)
...
def parse_method(name: str, run_config: dict) -> str:
    lowered = name.lower()
    for key in ('randomsampler', 'random_sampler', 'gomea', 'ga'):
...
def infer_model(name: str, run_config: dict) -> str:
    ...
    if model_id:
        return str(model_id)
    match = re.search('_clip_[^_]+_(sdxl|flux|pixart|lcm|sana|sana_sprint|sd)_([^_]+)_\\d+_', name.lower())
    if match:
...
def infer_backend(name: str, run_config: dict) -> str:
    for key in ('resolved_model_backend', 'model_backend'):
        ...
        if value:
            return str(value)
    match = re.search('_clip_[^_]+_(sdxl|flux|pixart|lcm|sana|sana_sprint|sd)_', name.lower())
    return match.group(1) if match else ''

def infer_optimization_target(run_config: dict) -> str:
    return str(run_config.get('resolved_optimization_target') or run_config.get('optimization_target') or '')
...
def infer_target_variable(name: str, run_config: dict, weights: Dict[str, int]) -> str:
    ...
    if len(nonzero) > 1:
        return '+'.join(nonzero)
    for metric, config_key in (('aesthetic_score', 'aesthetic_score_weight'), ('clip_score', 'clip_score_weight'), ('image_reward_score', 'image_reward_score_weight'), ('hpsv2_score', 'hpsv2_score_weight'), ('pickscore_score', 'pickscore_score_weight'), ('jpeg_size_kb', 'jpeg_size_weight')):
        try:
    ...
    if len(nonzero) > 1:
        return '+'.join(nonzero)
    lowered = name.lower()
    for token, metric in (('imagereward', 'image_reward_score'), ('image_reward', 'image_reward_score'), ('hpsv2', 'hpsv2_score'), ('pickscore', 'pickscore_score'), ('clip', 'clip_score'), ('aesthetic', 'aesthetic_score'), ('jpeg', 'jpeg_size_kb'), ('jpg', 'jpeg_size_kb')):
        if re.search(f'(?:^|_){token}(?:_|$)', lowered):
            return metric
    return ''
...
def prompt_dirs(experiment_dir: Path) -> List[Path]:
    return [path for path in sorted(experiment_dir.iterdir(), key=prompt_sort_key) if is_prompt_dir(path)]
...
def normalize_metrics(config_metrics, kind: str, df: pd.DataFrame) -> Dict[str, str]:
    ...
    if config_metrics is None or config_metrics == 'canonical':
        return {metric: column for metric, column in CANONICAL_METRIC_COLUMNS[kind].items() if column in available}
...
def stats(values: pd.Series) -> dict:
    ...
    if numeric.empty:
        return {'count': 0, 'min': np.nan, 'mean': np.nan, 'std': np.nan, 'median': np.nan, 'max': np.nan}
    return {'count': int(numeric.count()), 'min': float(numeric.min()), 'mean': float(numeric.mean()), 'std': float(numeric.std(ddof=0)) if numeric.count() > 1 else 0.0, 'median': float(numeric.median()), 'max': float(numeric.max())}
...
def experiment_metadata(experiment_dir: Path, run_config: dict) -> dict:
    ...
    return {'algorithm': DEFAULT_METHOD_LABELS.get(method_key, method_key), 'algorithm_key': method_key, 'model': infer_model(name, run_config), 'model_backend': infer_backend(name, run_config), 'target_variable': infer_target_variable(name, run_config, weights), 'optimization_target': infer_optimization_target(run_config), 'results_folder_name': name, 'results_folder_path': str(experiment_dir)}

def collect_prompt_values(experiment_dir: Path, row_selector: str, config_metrics) -> Tuple[pd.DataFrame, dict]:
    rows = []
    prompt_configs = []
    for prompt_dir in prompt_dirs(experiment_dir):
        ...
        values = {'prompt_folder': prompt_dir.name, 'csv_path': str(csv_path), 'csv_kind': kind, 'selected_row': row_selector, 'step_column': step_column, 'step': row.get(step_column, np.nan), 'prompt': first_non_empty(df, 'prompt') or '', 'category': first_non_empty(df, 'category') or ''}
...
def summarize_experiment(prompt_values: pd.DataFrame, metadata: dict) -> Tuple[List[dict], dict]:
    ...
    metric_names = [column for column in prompt_values.columns if column not in {'prompt_folder', 'csv_path', 'csv_kind', 'selected_row', 'step_column', 'step', 'prompt', 'category'} and pd.api.types.is_numeric_dtype(prompt_values[column])]
    long_rows = []
    wide_row = dict(base)
...
def autosize_excel_columns(writer: pd.ExcelWriter, sheet_name: str, df: pd.DataFrame) -> None:
    ...
    for idx, column in enumerate(df.columns, start=1):
        ...
        worksheet.column_dimensions[worksheet.cell(row=1, column=idx).column_letter].width = min(max(max_width + 2, 10), 60)
...
def build_quantitative_results(config: dict) -> Tuple[Path, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if importlib.util.find_spec('openpyxl') is None:
        raise RuntimeError(f'openpyxl is required to write the Excel workbook. CSV summaries were written to {out_dir}. Install project requirements or run `pip install openpyxl`.')
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Compile min/mean/std/median/max metrics for experiment folders.')
    parser.add_argument('--config', default='algorithms/config/config_process_quantitative_results.yaml', help='YAML config containing experiment_folders/source_dirs and output settings.')
