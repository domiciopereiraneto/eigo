...
from eigo_grid_search import _coerce_scalar, create_run_config, extract_final_results, get_prompt_list, load_yaml, save_run_parameters
from src.optimization_targets import resolve_optimization_target
DEFAULT_CONFIG = 'algorithms/config/config_eigo_optuna_search.yaml'
SUPPORTED_METHODS = ('ga', 'gomea', 'random_sampler')
AUTO_METRICS = {'ga': 'best_fitness', 'gomea': 'best_fitness', 'random_sampler': 'best_fitness'}
DEFAULT_OPTUNA_PLOTS = {'optimization_history': 'plot_optimization_history', 'param_importances': 'plot_param_importances', 'slice': 'plot_slice', 'contour': 'plot_contour', 'parallel_coordinate': 'plot_parallel_coordinate', 'edf': 'plot_edf', 'timeline': 'plot_timeline', 'intermediate_values': 'plot_intermediate_values', 'rank': 'plot_rank'}

def get_enabled_methods(config):
    methods = [method for method in SUPPORTED_METHODS if config.get('test_' + method, method == 'ga')]
    if not methods:
...
def _sample_diffusiondb_prompts(prompt_source):
    try:
        ...
        raise ImportError("The 'datasets' package is required to sample prompts from DiffusionDB.") from exc
    ...
    print(f'Loading DiffusionDB prompts (dataset={dataset_name}, subset={subset}, split={split}, count={count}, seed={seed})')
    ...
    if streaming:
        ...
        prompts = []
        for row in dataset:
            _append_prompt_if_valid(prompts, skipped, row.get(prompt_column), require_clip_tokenizable, clip_context_length)
            if len(prompts) >= count:
                break
    else:
        if prompt_column not in dataset.column_names:
            raise ValueError(f"Prompt column '{prompt_column}' not found in DiffusionDB split. Available columns: {', '.join(dataset.column_names)}")
        candidates = list(dataset[prompt_column])
        rng = random.Random(seed)
        rng.shuffle(candidates)
        prompts = []
        for prompt in candidates:
            _append_prompt_if_valid(prompts, skipped, prompt, require_clip_tokenizable, clip_context_length)
            if len(prompts) >= count:
                break
    if len(prompts) < count:
        raise ValueError(f'DiffusionDB prompt source yielded {len(prompts)} prompt(s), but {count} were requested. Skipped prompts: {skipped}.')
    if skipped['too_long_for_clip']:
        print(f"Skipped {skipped['too_long_for_clip']} DiffusionDB prompt(s) longer than CLIP context length {clip_context_length}.")
    return prompts
...
def get_optuna_prompt_list(config):
    ...
    raise ValueError(f"Unsupported 'optuna.prompt_source.type': {source_type}. Supported values: config, diffusiondb.")
...
def validate_optuna_config(config):
    ...
    if unknown_methods:
        raise ValueError(f"'optuna.search_space' contains unsupported method sections: {', '.join(unknown_methods)}")
...
def suggest_parameter(trial, name, spec):
    ...
    if param_type == 'float':
        ...
        return trial.suggest_float(name, float(low), float(high), log=bool(spec.get('log', False)), step=_coerce_scalar(spec.get('step')))
    if param_type == 'int':
        low = _coerce_scalar(spec.get('low'))
        high = _coerce_scalar(spec.get('high'))
        if low is None or high is None:
            raise ValueError(f"Int parameter '{name}' must define low and high.")
        return trial.suggest_int(name, int(low), int(high), log=bool(spec.get('log', False)), step=int(_coerce_scalar(spec.get('step', 1))))
...
def suggest_overrides(trial, method, search_space):
    ...
    for key, spec in method_space.items():
        if isinstance(spec, dict) and (not condition_matches(overrides, spec.get('depends_on'))):
            continue
        overrides[key] = suggest_parameter(trial, f'{method}.{key}', parameter_spec_without_condition(spec))
    return overrides
...
def run_eigo_method(run_config, method):
    ...
    if method == 'gomea':
        return eigo_engine.run_gomea_optimization()
...
def save_optuna_trial_parameters(results_folder, run_id, method, overrides, run_config, trial_number):
    ...
    with optuna_params_path.open('w', encoding='utf-8') as f:
        yaml.safe_dump({'trial_number': trial_number, 'run_id': run_id, 'method': method, 'selected_prompt': run_config.get('selected_prompt'), 'overrides': overrides, 'effective_parameters': {key: run_config.get(key) for key in sorted(set(overrides.keys()) | {'optimization_target'})}}, f, sort_keys=False)
...
def run_optuna_search(config, dry_run=False):
    ...
    try:
        ...
        raise ImportError('Optuna is required for this script. Install it with `pip install optuna` or update the project environment.') from exc
    ...
    study = optuna.create_study(direction=direction, sampler=create_sampler(optuna_config), study_name=study_name, storage=storage, load_if_exists=load_if_exists)

    def objective(trial):
        method = enabled_methods[0] if len(enabled_methods) == 1 else trial.suggest_categorical('method', enabled_methods)
        overrides = suggest_overrides(trial, method, search_space)
        values = []
        prompt_results = []
        for prompt_idx, prompt in enumerate(prompt_list, start=1):
            run_id = f'trial_{trial.number:04d}_prompt_{prompt_idx:03d}_{method}_{timestamp}'
            ...
            _, optuna_params_path = save_optuna_trial_parameters(run_config['results_folder'], run_id, method, overrides, run_config, trial.number)
            ...
            values.append(value)
            save_optuna_trial_parameters(produced_folder, run_id, method, overrides, run_config, trial.number)
            prompt_results.append({'prompt_index': prompt_idx, 'selected_prompt': prompt, 'run_id': run_id, 'results_folder': run_config['results_folder'], 'produced_folder': produced_folder, 'metric': resolved_metric, 'value': value, 'optimization_target': resolved_optimization_target, 'final_results': final_results})
        ...
        row = {'trial_number': trial.number, 'method': method, 'overrides': copy.deepcopy(overrides), 'objective_value': objective_value, 'prompt_results': prompt_results}
...
def _best_trial_payload(study):
    ...
    try:
        ...
        return None
    return {'number': best_trial.number, 'value': best_trial.value, 'params': dict(best_trial.params), 'user_attrs': dict(best_trial.user_attrs)}
...
def save_summary(config, study, trial_rows):
    ...
    optuna_config = get_optuna_config(config)
    payload = {'selected_prompts': config.get('_resolved_optuna_prompts') or get_optuna_prompt_list(config), 'prompt_source': get_optuna_prompt_source(config), 'enabled_methods': get_enabled_methods(config), 'default_optimization_target': resolve_optimization_target(config), 'n_trials': int(optuna_config.get('n_trials', 20)), 'direction': optuna_config.get('direction', 'maximize'), 'metric': optuna_config.get('metric', 'auto'), 'study_name': None if study is None else study.study_name, 'best_trial': _best_trial_payload(study), 'completed_trial_runs': trial_rows}
...
def _get_enabled_plot_names(plot_config):
    ...
    if isinstance(enabled, list):
        ...
        if unknown:
            raise ValueError(f"'optuna.plots.enabled' contains unsupported plot names: {', '.join(unknown)}")
        return list(enabled)
...
def save_optuna_plots(config, study):
    ...
    for plot_name in plot_names:
        ...
        if plot_function is None:
            failed.append({'plot': plot_name, 'error': f'optuna.visualization.{function_name} is unavailable'})
            continue
        try:
    ...
    with manifest_path.open('w', encoding='utf-8') as f:
        yaml.safe_dump({'study_name': study.study_name, 'completed_trials': len(completed_trials), 'saved': saved, 'failed': failed}, f, sort_keys=False)
    if saved:
...
def build_parser():
    parser = argparse.ArgumentParser(description='Run Optuna hyperparameter search for EIGO with one or more prompts.')
    parser.add_argument('--config', type=str, default=DEFAULT_CONFIG, help='Path to the Optuna-search config YAML.')
    parser.add_argument('--dry-run', action='store_true', help='Validate the config and print search spaces without executing optimization.')
    return parser
